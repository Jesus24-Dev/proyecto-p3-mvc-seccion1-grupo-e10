...
export async function seedOrders() {
    const userEmails = [...new Set(orderSeedData.map((order) => order.userEmail))];
    const agencyNames = [
        ...new Set(
            orderSeedData.flatMap((order) => [order.originAgencyName, order.destinationAgencyName]),
        ),
    ];
    ...
    const agencyByName = new Map(agencies.map((agency) => [agency.name, agency.id]));
    const seededOrders = [];
    ...
    for (const order of orderSeedData) {
        ...
        if (!userId) {
            throw new Error(`Cannot seed order ${order.description}: missing user ${order.userEmail}.`);
        }

        if (!originAgencyId || !destinationAgencyId) {
            throw new Error(`Cannot seed order ${order.description}: missing origin or destination agency.`);
        }
